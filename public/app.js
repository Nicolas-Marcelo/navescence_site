...
async function loadActiveAlerts() {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        "Não foi possível carregar os alertas ativos."
      );
    }

    const data = await response.json();

    persistentAlerts = Array.isArray(data)
      ? data
      : [];
    ...
    console.error(
      "Erro ao carregar alertas ativos:",
      error
    );
  }
}
...
function renderPersistentAlerts() {
  const count = persistentAlerts.length;

  activeAlerts.textContent = count;
  ...
  renderAlerts(persistentAlerts);
}

function renderVerification(nodes) {
  const verifyingEntry = nodes.find(
    ([, data]) => data.status === "VERIFICANDO"
  );
  ...
  if (!verifyingEntry) {
    ...
    verificationDescription.textContent =
      "Nenhum sensor está sendo verificado neste momento.";

    verificationState.textContent =
      "Aguardando";

    verificationPulse.className =
      "pulse idle";
    ...
    alertsContainer.innerHTML = `
      <div class="no-alerts">
        ...
        <div>
          ...
          <p>
            A infraestrutura está operando normalmente.
          </p>
        </div>
      </div>
    `;
...
function renderSensors() {
  ...
  if (query) {
    nodes = nodes.filter(
      ([nodeName, data]) => {
        const text = `
          ${nodeName}
          ${getExpectedNeighbors(data)}
          ${getDetectedNeighbors(data)}
        `.toLowerCase();
  ...
  visibleNodesCount.textContent =
    `${nodes.length} ${nodes.length === 1 ? "sensor" : "sensores"}`;
...
function createSensorRow(nodeName, data) {
  ...
  const presence =
    formatPresence(data.presence);

  const status =
    formatStatus(data.status);

  const diagnostic =
    formatDiagnostic(data.diagnostic);
  ...
  row.innerHTML = `
    ...
    <td>
      ${formatDate(data.lastCommunication)}
    </td>

    <td>
      <span class="diagnostic-label ${diagnostic.className}">
        ${diagnostic.text}
      </span>
    </td>
...
sensorSearch.addEventListener(
  "input",
  renderSensors
);

presenceFilter.addEventListener(
  "change",
  renderSensors
);

diagnosticFilter.addEventListener(
  "change",
  renderSensors
);
...
function renderDrawer(nodeName) {
  const data =
    currentDashboard.nodes?.[nodeName];

  if (!data) return;

  drawerNodeName.textContent =
    nodeName;
  ...
  const neighbors =
    Array.isArray(data.detectedNeighbors)
      ? data.detectedNeighbors
      : [];
    ...
    <section class="drawer-section">
      ...
      <div class="drawer-info">
        ...
        <div class="drawer-row">
          ...
          <strong>
            ${formatDate(data.lastVerification)}
          </strong>
        </div>
      </div>
    </section>
...
function getExpectedNeighbors(data) {
  if (
    Array.isArray(
      data.expectedNeighbors
    )
  ) {
    return (
      data.expectedNeighbors.join(", ") ||
      "—"
    );
  }

  return (
    data.expectedNeighbor ??
    "—"
  );
}
